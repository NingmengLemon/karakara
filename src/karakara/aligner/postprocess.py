...
实测规模（6 首歌、1497 个单元的真实运行日志）：337 个零长度，占 **22.5%**，
逐首 10.7% / 15.3% / **41.8%** / 24.6%；所有单元跨度都是 80ms 的整数倍；
70% 落在行中，80% 紧跟前一个单元的结束时刻。

零长度单元在产物里的表现是「与前一个标签完全重复的标签」，播放器无法单独高亮它。
本模块负责两件事：

* :func:`zero_length_stats` / :func:`zero_length_ratio` —— 把它作为**质量信号**报出来；
...
token（假名不算 ``is_cjk_char``），比原来还粗。详见 ``docs/aligner.md``。
"""
...
#: 零长度占比超过这个值就告警。实测：Saya 10.7% / SACRA 15.3% / samples 24.6%
#: 属"正常"范围，蒲公英 41.8% 则明显是模型在这首歌上吃力。
