...
4. **一道机械防漂移测试**（`tests/test_aligner_backends.py`）：登记表里的脚本要存在、
   脚本里的 `DEFAULT_PORT`/`DEFAULT_HOST` 要与登记表一致、两个后端不能撞端口、
   **登记表声称支持的语言要等于服务端真正接受的语言**。这个项目踩过
   「客户端默认 8787 / 服务端默认 8000」的坑，多后端之后更该有这道检查；
   语言那条则是为了让 `--aligner-language ko` 配 `hfa` 在**开跑前**就失败，
...
与 HFA 一致，作为对照与回退仍有价值，而且它不依赖 `models/aligner/HubertFA/` 那 399MB
模型与 `third_party/HubertFA` 这个 submodule。

### 9.7 V4：扰动稳定性（2026-09-18，两个后端都跑了）

§8 的 V4 判据**事先定好**：行内边界漂移的**中位数 < 1 个帧移**视为稳定。下面一律按
...
1. `pad±` 的漂移**先扣掉已知平移**。+100ms 的片段里，同一条边界理应出现在 `base+100ms`；
   不扣掉的话量到的是"我加的 padding"，而不是"对齐器的敏感度"。
2. 只比**行内**边界（首尾各去掉一条）。首条边界是"人声从哪开始"、末条边界贴着片段尾，
   它们的差属于**锚定误差**，单独报（第 4 条会说这个指标对 HFA 为什么不适用）。
3. 单元数变了（`drop1`/`add1`）就不做逐边界比较，改报"是否还成立"。

**输入**：中文 `samples/countdown_to_zero_luotianyi`（20 行）、日文 `D:\MUSIC\Cryu - 蒲公英`
（17 行，就是 §9.6 人耳裁决那一首）。同一份分离人声、同一份行切分、同一份行文本；
分离人声用 `UVR_Demucs_Model_1` 现跑一次并缓存。

**中文（20 行 / 139 条行内边界）**，单元格是 `中位 / p90 / 最大`（ms）：

| 条件 | HubertFA | Qwen3 | 10ms 判据 |
|---|---|---|---|
| `pad+100ms` | **1 / 41 / 740** | 20 / 60 / 1060 | HFA 稳定；Qwen 不稳定 |
...
| 条件 | HubertFA | Qwen3 | 10ms 判据 |
|---|---|---|---|
| `pad+100ms` | **1 / 13 / 122** | 60 / 2540 / 6340 | HFA 稳定；Qwen 不稳定 |
| `pad-100ms` | **1 / 12 / 110** | 60 / 420 / 6380 | 同上 |
| 混音 vs 分离人声 | **18.5 / 398 / 1643** | 1280 / 5920 / 10480 | 两者都远大于一个帧移 |

五条结论：

1. **片段边界 ±100ms 对 HFA 基本无影响**（日文中位 1ms、p90 12–13ms）。所以 §4 里那句
   "逐行送片段时建议两端各留 100–200ms padding" **对本项目的默认后端不需要**——生产路径
   的紧切片段就是对的，加 padding 只会多算音频。对照之下 Qwen 在日文上**中位就漂 60ms**，
   是它自己 80ms 量化格的 3/4：它的边界本来就在抖，而不是"量化得粗但稳定"。
2. **真正的主导变量是分离质量，不是片段边界。** 换成原始混音后两个后端的漂移都远大于
   帧移（HFA ja 中位 18.5ms / p90 398ms；Qwen ja 中位 1280ms / p90 5920ms）。这与 §9.1
   一致：模型在歌声上的退化是主因。**要再提高质量，优先换更好的分离模型，而不是调切段。**
3. **错一个字的代价是"从错处往后全毁"，不是"整行糊掉"。** 扰动点**之前**的边界漂移在日文上
...
5. **顺手核对到的单元粒度**：HFA 服务返回的单元是 **zh 逐字、ja 按 `pykakasi` 的词、
   en 按词**，不是音素/モーラ。所以 §9.2b 表里的"260 モーラ"是 TextGrid **音素层**统计出来的，
   产物里的单元数要少得多（`蒲公英` 这一行在产物里是 1 个单元）。

**对默认值决策的意义**：§9.6 证明的是"某一行的对错"，V4 证明的是"整首歌的稳定性"。
日文上 HFA 三项全面更稳；中文上两者互有胜负（`pad` HFA 更稳，`mix` 的中位 Qwen 更好但
p90 更差）。合起来支持"默认 hfa"。
